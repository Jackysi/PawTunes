...
function compileScss(files) {
    const tasks = files.map((file) => {
        return gulp.src(file.src)
            ...
            .pipe(gulp.dest(file.dest)).on('error', function (err) {
...
function minifyJS() {
    const tasks = jsFiles.map((file) => {
        return gulp.src(file.src)
            .pipe(sourcemaps.init())
            .pipe(concat(file.outputName)).pipe(
                uglify({
                    output: {
                        preamble: '"use strict";\n',
                    },
...
export const build = gulp.series(
    gulp.parallel(compileControlPanelSCSSFiles, compileTemplateSCSSFiles, minifyJS, compileTypeScript),
);
